...
if ( ! function_exists('oGeoLocation')){
    function oGeoLocation(){
        //return geoip('45.114.89.17');
        $ip = get_the_user_ip();
...
if ( ! function_exists('defaultLatLon')){
...
if ( ! function_exists('currentCountry')){
    function currentCountry(){
        ...
        if (session('country')){
            ...
            $country = \App\Country::whereCountryCode($iso_code)->first();
            if ($country){
                session(['country' => $country->toArray()]);
                $current_country = $country->toArray();
            }else{
                $country = \App\Country::whereCountryCode('US')->first();
                $current_country = $country->toArray();
            }
        }
        ...
        if (env('APP_DEMO') == true) {
            ...
            die(var_dump($iso_code));
            die(var_dump(session('country')));
...
if ( ! function_exists('currentStates')){
    function currentStates(){
        $currentCountry = currentCountry();
        $country = \App\Country::whereCountryCode($currentCountry['country_code'])->with('states')->first();
